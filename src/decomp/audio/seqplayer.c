    ...
    for (i = 0; i < CHANNELS_MAX; i++) {
        if (channelBits & 1) {
            ...
            if (IS_SEQUENCE_CHANNEL_VALID(seqChannel) == FALSE) {
                DEBUG_PRINT("Audio:Track:Warning: No Free Notetrack\n");
...
void audio_list_push_back(struct AudioListItem *list, struct AudioListItem *item) {
    if (item->prev != NULL) {
        DEBUG_PRINT("Error:Same List Add\n");
    } else {
        list->prev->next = item;
        item->prev = list->prev;
        item->next = list;
    ...
    for (;;) {
        ...
        switch (cmd) {
            ...
            case 0xfc: // layer_call
                if (0 && state->depth >= 4) {
                    DEBUG_PRINT("Macro Level Over Error!\n");
                }
            ...
            case 0xc6: // layer_setinstr
                ...
                if ((layer->instOrWave = get_instrument(seqChannel, cmd, &layer->instrument, &layer->adsr)) == 0) {
                    DEBUG_PRINT("WARNING: NPRG: cannot change %d\n", cmd);
                    layer->instOrWave = 0xff;
                }
#endif
                break;
            ...
            default:
                switch (cmd & 0xf0) {
                    ...
                    case 0xe0: // layer_setshortnotedurationfromtable
                        ...
                        break;
                    default:
                        DEBUG_PRINT("Audio:Track:NOTE:UNDEFINED NOTE COM. %x\n", cmd);
                        break;
                }
        }
    }
        ...
        for (;;) {
...
#endif
            {
                switch (cmd) {
                    ...
                    case 0xfc: // chan_call
                        if (0 && state->depth >= 4) {
                            DEBUG_PRINT("Audio:Track :Call Macro Level Over Error!\n");
                        }
                    ...
                    case 0xf8: // chan_loop; loop start, N iterations (or 256 if N = 0)
                        if (0 && state->depth >= 4) {
                            DEBUG_PRINT("Audio:Track :Loops Macro Level Over Error!\n");
                        }
                        state->remLoopIters[state->depth] = m64_read_u8(state);
#if defined(VERSION_EU) || defined(VERSION_SH)
                        state->stack[state->depth++] = state->pc;
                    ...
                    case 0xeb: // chan_setbankandinstr
...
#endif
                        {
                            ...
                            DEBUG_PRINT("SUB:ERR:BANK %d NOT CACHED.\n", cmd);
                        }
                        // fallthrough
#endif
                    ...
                    case 0xe4: // chan_dyncall
                        if (value != -1) {
#if defined(VERSION_EU) || defined(VERSION_SH)
                            if (state->depth >= 4) {
                                DEBUG_PRINT("Audio:Track: CTBLCALL Macro Level Over Error!\n");
                            }
#endif
                            seqData = (*seqChannel->dynTable)[value];
#if defined(VERSION_EU) || defined(VERSION_SH)
                            ...
                            if (0 && sp38 >= gSeqFileHeader->seqArray[seqPlayer->seqId].len) {
                                DEBUG_PRINT("Err :Sub %x ,address %x:Undefined SubTrack Function %x", seqChannel, state->pc, sp38);
                            }
#else
                            state->depth++, state->stack[state->depth - 1] = state->pc;
...
void sequence_player_process_sequence(struct SequencePlayer *seqPlayer) {
    ...
    if (IS_SEQ_LOAD_COMPLETE(seqPlayer->seqId) == FALSE
        ...
        DEBUG_PRINT("Disappear Sequence or Bank %d\n", seqPlayer->seqId);
        ...
        for (;;) {
            ...
            if (cmd >= 0xc0) {
                switch (cmd) {
                    ...
                    default:
                        DEBUG_PRINT("Group:Undefine upper C0h command (%x)\n", cmd);
                        break;
                }
            } else {
                loBits = cmd & 0xf;
                switch (cmd & 0xf0) {
                    ...
                    default:
                        DEBUG_PRINT("Group:Undefined Command\n");
                        break;
                }
            }
        }
...
void init_sequence_player(u32 player) {
    DEBUG_PRINT("init sequence player");
...
#define LAYERS_SIZE ARRAY_COUNT(gSequenceLayers)
    ...
    for (i = 0; i < SEQUENCE_PLAYERS; i++) {
        for (j = 0; j < CHANNELS_MAX; j++) {
            DEBUG_PRINT("seq %d channel %d", i,j);
