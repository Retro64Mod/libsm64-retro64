...
void audio_dma_copy_immediate(uintptr_t devAddr, void *vAddr, size_t nbytes) {
    DEBUG_PRINT("Romcopy %x -> %x ,size %x\n", devAddr, vAddr, nbytes);
...
void *sequence_dma_async(s32 seqId, s32 arg1, struct SequencePlayer *seqPlayer) {
    ...
    DEBUG_PRINT("Seq %d Loading Start\n", seqId);
    seqLength = gSeqFileHeader->seqArray[seqId].len + 0xf;
    seqLength = ALIGN16(seqLength);
    seqData = gSeqFileHeader->seqArray[seqId].offset;
    ptr = alloc_bank_or_seq(&gSeqLoadedPool, 1, seqLength, arg1, seqId);
    if (ptr == NULL) {
        DEBUG_PRINT("Heap Overflow Error\n");
        return NULL;
    }
...
void preload_sequence(u32 seqId, u8 preloadMask) {
    ...
    if (preloadMask & PRELOAD_SEQUENCE) {
        ...
        if (IS_BANK_LOAD_COMPLETE(seqId) == TRUE) {
            DEBUG_PRINT("SEQ  %d ALREADY CACHED\n", seqId);
...
void load_sequence_internal(u32 player, u32 seqId, s32 loadAsync) {
    ...
    if (loadAsync) {
        ...
        if (numMissingBanks == 1) {
            DEBUG_PRINT("Ok,one bank slow load Start \n");
            ...
            DEBUG_PRINT("Sorry,too many %d bank is none.fast load Start \n", numMissingBanks);
    ...
    DEBUG_PRINT("Seq %d:Default Load Id is %d\n", seqId, seqPlayer->defaultBank[0]);
    DEBUG_PRINT("Seq Loading Start\n");

    seqPlayer->seqId = seqId;
    sequenceData = get_bank_or_seq(&gSeqLoadedPool, 2, seqId);
    if (sequenceData == NULL) {
        if (seqPlayer->seqDmaInProgress) {
            DEBUG_PRINT("Error:Before Sequence-SlowDma remain.\n");
            DEBUG_PRINT("      Cancel Seq Start.\n");
            return;
        }
        if (loadAsync) {
...
void audio_init() {
...
#ifdef TARGET_N64
    DEBUG_PRINT("Clear Workarea %x -%x size %x \n",
    ...
    DEBUG_PRINT("Heap reset.Synth Change %x \n", 0);
    DEBUG_PRINT("Heap %x %x %x\n", 0, 0, 0);
    DEBUG_PRINT("Main Heap Initialize.\n");
    ...
    DEBUG_PRINT(" Syndrv    :[%6d]\n", 0); // gSoundDataADSR
    DEBUG_PRINT(" Seqdrv    :[%6d]\n", 0); // gMusicData
    DEBUG_PRINT(" audiodata :[%6d]\n", 0); // gSoundDataRaw
    DEBUG_PRINT("---------------------------------------\n");
}
#endif
