...
GeoLayout* convertPtr_follow(unsigned char* rom,unsigned int oldPtr,unsigned char* MI0_data){
    ...
    GeoLayout* parsed=parse_full_geolayout(rom,rom+newPtr,MI0_data);
    return parsed;
}
...
GeoLayout* parse_full_geolayout(unsigned char* rom,unsigned char* cmdData, unsigned char* MI0_data){
    ...
    bool done=false;
    int len=0;
    unsigned char* currentLayout=cmdData;
    while(!done){
        done=parse_command(rom,currentLayout,buffer,&len,MI0_data);
...
_Bool parse_command(unsigned char* rom,unsigned char* cmd,GeoLayout* buf,uint8_t *cmd_length,unsigned char* MI0_data){
    *cmd_length = 0;
    if (cmd[0]==0 && cmd[1]==0 && cmd[2]==0 && cmd[3]==0){
        ...
        unsigned int segmented_address = read_u32_be(cmd+4);
        GEO_DEBUG_PRINT("Branch and Store: %i",segmented_address);
        *cmd_length=8;
        paste_macro2(buf,GEO_BRANCH_AND_LINK(convertPtr_follow(rom,segmented_address,MI0_data)));
    }else if (cmd[0]==1 && cmd[1]==0 && cmd[2]==0 && cmd[3]==0){
        ...
        *cmd_length=4;
        ...
        return true;
    }else if (cmd[0]==2 && cmd[2]==0 && cmd[3]==0){
    ...
    }else if (cmd[0]==0x0E && cmd[1]==0 && cmd[2]==0){
        // GeoLayout command 0E: Switch case. 0E 00 00 [NN] [AA AA AA AA]
        // N = Number of cases/display lists, starting at 01
        // AA = ASM function
        unsigned char num_cases = cmd[3];
        unsigned int asm_func = read_u32_be(cmd+4);
        *cmd_length=8;
    ...
    // 0F: Create Camera Graph Node (unused for libsm64)
    else if (cmd[0]==0x10 && cmd[1]==0 && cmd[2]==0 && cmd[3]==0){
        ...
        //RY	Y rotation (s16)
        //RZ	Z rotation (s16)
        unsigned char branch_flag = cmd[1];
        unsigned short branch_flag_2 = read_u16_be(cmd+2);
        short x_translation = read_u16_be(cmd+4);
        short y_translation = read_u16_be(cmd+6);
        short z_translation = read_u16_be(cmd+8);
        short x_rotation = read_u16_be(cmd+10);
        short y_rotation = read_u16_be(cmd+12);
        short z_rotation = read_u16_be(cmd+14);
        ...
        *cmd_length=0x10;
        ...
        unsigned char branch_flag = FIRST_NIBBLE(cmd[1]);
        unsigned char layer = SECOND_NIBBLE(cmd[1]);
        unsigned short x_translation = read_u16_be(cmd+2);
        unsigned short y_translation = read_u16_be(cmd+4);
        unsigned short z_translation = read_u16_be(cmd+6);
        unsigned int segmented_address = read_u32_be(cmd+8);
        *cmd_length=0x8;
        GEO_DEBUG_PRINT("Translate Node & load DL/GL: %i, %i, %i, %i, %i",branch_flag,layer,x_translation,y_translation,z_translation);
        if (branch_flag==0x8){
            *cmd_length=0xC;
            paste_macro3(buf, GEO_TRANSLATE_WITH_DL(layer, x_translation, y_translation, z_translation, convertDLPtrMD(MI0_data,segmented_address)));
            return false;
        }
        ...
        unsigned char branch_flag = FIRST_NIBBLE(cmd[1]);
        unsigned char layer = SECOND_NIBBLE(cmd[1]);
        unsigned short x_rotation = read_u16_be(cmd+2);
        unsigned short y_rotation = read_u16_be(cmd+4);
        unsigned short z_rotation = read_u16_be(cmd+6);
        unsigned int segmented_address = read_u32_be(cmd+8);
        *cmd_length=0x8;
        GEO_DEBUG_PRINT("Rotate Node: %i, %i, %i, %i, %i",branch_flag,layer,x_rotation,y_rotation,z_rotation);
        if (branch_flag==0x8){
            *cmd_length=0xC;
            paste_macro3(buf, GEO_ROTATION_NODE_WITH_DL(layer, x_rotation, y_rotation, z_rotation, convertDLPtrMD(MI0_data,segmented_address)));
            return false;
        }
        ...
        unsigned char layer = cmd[1];
        unsigned int x_offset = read_u16_be(cmd+2);
        unsigned int y_offset = read_u16_be(cmd+4);
        unsigned int z_offset = read_u16_be(cmd+6);
        unsigned int segmented_address = read_u32_be(cmd+8);
        *cmd_length=0xC;
        GEO_DEBUG_PRINT("Load Display List With Offset: %i, %i, %i, %i, %X",layer,x_offset,y_offset,z_offset,segmented_address);
        paste_macro3(buf, GEO_ANIMATED_PART(layer, x_offset, y_offset, z_offset, convertDLPtrMD(MI0_data,segmented_address)));
    }else if (cmd[0]==0x14){
        ...
        *cmd_length=0x8;
        ...
        if (branch_flag==0x8){
            *cmd_length=0xC;
            paste_macro3(buf, GEO_BILLBOARD_WITH_PARAMS_AND_DL(layer, x_translation, y_translation, z_translation, convertDLPtrMD(MI0_data,segmented_address)));
            return false;
        }
        ...
        *cmd_length=0x8;
        ...
        paste_macro2(buf, GEO_DISPLAY_LIST(layer, convertDLPtrMD(MI0_data,segmented_address)));
    }else if (cmd[0]==0x16){
        ...
        *cmd_length=0x4;
        ...
        //A ASM RAM Address
        unsigned short parameters = read_u16_be(cmd+2);
        unsigned int asm_address = read_u32_be(cmd+4);
        *cmd_length=0x8;
    ...
    // 0x19 = Set background, not needed in libsm64
    else if (cmd[0]==0x1A || cmd[0]==0x1E || cmd[0]==0x1F){
        ...
        *cmd_length=0x8;
        ...
        unsigned char param=cmd[1];
        short offsetX = read_u16_be(cmd+2);
        short offsetY = read_u16_be(cmd+4);
        short offsetZ = read_u16_be(cmd+6);
        unsigned int asm_address = read_u32_be(cmd+8);
        *cmd_length=0xC;
        ...
        unsigned char a = FIRST_NIBBLE(cmd[1]);
        unsigned char b = SECOND_NIBBLE(cmd[1]);
        unsigned int scale = read_u32_be(cmd+4);
        unsigned int d = read_u32_be(cmd+8);
        if (a==0){
            *cmd_length=0x8;
            ...
            *cmd_length=0xC;
            ...
            paste_macro3(buf, GEO_SCALE_WITH_DL(b,scale,convertDLPtrMD(MI0_data,d)));
            return false;
        }
    }else{
        GEO_DEBUG_PRINT("Unknown GEO command.. first 4 bytes: %x %x %x %x", cmd[0], cmd[1], cmd[2], cmd[3]);
    }
