...
BAD_RETURN(f32) calc_y_to_curr_floor(f32 *posOff, f32 posMul, f32 posBound, f32 *focOff, f32 focMul, f32 focBound) {
    ...
    if (sMarioCamState->action & ACT_FLAG_ON_POLE) {
        if (sMarioGeometry.currFloorHeight >= g_state->mgMarioStateVal.usedObj->oPosY && sMarioCamState->pos[1]
                   < 0.7f * g_state->mgMarioStateVal.usedObj->hitboxHeight + g_state->mgMarioStateVal.usedObj->oPosY) {
            posBound = 1200;
        }
    }
    ...
    if (!(gCameraMovementFlags & CAM_MOVE_ROTATE)) {
        ...
        if (avoidStatus == 3) {
            ...
            if (c->mode == CAMERA_MODE_RADIAL) {
                ...
                rotateSpeed = g_state->mgMarioStateVal.forwardVel / 32.f * 128.f;
...
s32 update_boss_fight_camera(struct Camera *c, Vec3f focus, Vec3f pos) {
    ...
    /*if (heldState == 1) {
        focus[1] += 300.f * sins((g_state->mgMarioStateVal.angleVel[1] > 0.f) ?  g_state->mgMarioStateVal.angleVel[1]
                                                                     : -g_state->mgMarioStateVal.angleVel[1]);
    }*/
...
s32 mode_behind_mario(struct Camera *c) {
    struct MarioState *marioState = &g_state->mgMarioStateVal;
    struct Surface *floor;
    Vec3f newPos;
...
s16 update_slide_camera(struct Camera *c) {
    ...
    s16 camPitch;
    s16 camYaw;
    UNUSED struct MarioState *marioState = &g_state->mgMarioStateVal;
...
s16 update_default_camera(struct Camera *c) {
    ...
    if (avoidStatus == 3) {
        ...
        if (g_state->mgMarioStateVal.forwardVel == 0.f) {
    ...
    if (sMarioCamState->action & ACT_FLAG_ON_POLE) {
        camFloorHeight = g_state->mgMarioStateVal.usedObj->oPosY + 125.f;
...
void move_mario_head_c_up(UNUSED struct Camera *c) {
    ...
    sCUpCameraPitch += (s16)(gMarioState->controller->stickY * 10.f);
    sModeOffsetYaw -= (s16)(gMarioState->controller->stickX * 10.f);
...
void update_camera(struct Camera *c) {
    ...
    stub_camera_3(c);
    sCButtonsPressed = find_c_buttons_pressed(sCButtonsPressed, gMarioState->controller->buttonPressed,
                                              gMarioState->controller->buttonDown);
    ...
    if (gCurrLevelNum != LEVEL_CASTLE) {
        ...
        if ((c->cutscene == 0 &&
            ...
            if (c->cutscene == 0 && (gMarioState->controller->buttonPressed & R_TRIG)
...
s32 offset_yaw_outward_radial(struct Camera *c, s16 areaYaw) {
    ...
    dYaw = g_state->mgMarioStateVal.forwardVel / 32.f * 128.f;
...
void play_camera_buzz_if_c_sideways(void) {
    if ((gMarioState->controller->buttonPressed & L_CBUTTONS)
        || (gMarioState->controller->buttonPressed & R_CBUTTONS)) {
        play_sound_button_change_blocked();
    }
}
...
void warp_camera(f32 displacementX, f32 displacementY, f32 displacementZ) {
    Vec3f displacement;
    struct MarioState *marioStates = &g_state->mgMarioStateVal;
    struct LinearTransitionPoint *start = &sModeInfo.transitionStart;
    struct LinearTransitionPoint *end = &sModeInfo.transitionEnd;
...
BAD_RETURN(s32) cutscene_dialog_start(struct Camera *c) {
    ...
    //set_time_stop_flags(TIME_STOP_ENABLED | TIME_STOP_DIALOG); libsm64 TODO: handle this?
...
BAD_RETURN(s32) cutscene_dialog_end(struct Camera *c) {
    sStatusFlags |= CAM_FLAG_UNUSED_CUTSCENE_ACTIVE;
    c->cutscene = 0;
    //clear_time_stop_flags(TIME_STOP_ENABLED | TIME_STOP_DIALOG); libsm64 TODO maybe handle this?
}
    ...
    switch (sCutsceneVars[0].angle[0]) {
        ...
        case 0:
            if (get_dialog_id() != -1) {
                ...
                // set_time_stop_flags(TIME_STOP_ENABLED | TIME_STOP_DIALOG); libsm64 TODO maybe handle this?
            }
            break;
        // Leave the dialog.
        case 1:
